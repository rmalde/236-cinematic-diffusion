...
finetuned_model_path = "dreambooth_model"

prompts = [
    "will smith slapping chris rock",
    "john cena turning invisible",
    "titanic but as a human",
    "US presidents working out at the gym",
    "an underwater airplane",
]

pipeline_base = StableDiffusionPipeline.from_pretrained(base_model_path, torch_dtype=torch.float16)
pipeline_base.to("cuda")

pipeline_finetuned = DiffusionPipeline.from_pretrained(finetuned_model_path, torch_dtype=torch.float16, use_safetensors=True)
...
for i, prompt in enumerate(prompts):
    ...
    image_b = pipeline_finetuned("sks " + prompt).images[0]
